...
def resolve_config_paths() -> ConfigPaths:
    ...
    try:
        ...
        desc_share = Path(get_package_share_directory('ur5e_description'))
        mv_share = Path(get_package_share_directory('ur5e_moveit_config'))
        bringup_share = Path(get_package_share_directory('ur5e_bringup'))
    except (ImportError, LookupError):
        src_root = Path(__file__).resolve().parents[2].parent
        desc_share = src_root / 'ur5e_description'
        mv_share = src_root / 'ur5e_moveit_config'
        bringup_share = src_root / 'ur5e_bringup'

    controllers = desc_share / 'config' / 'ros2_controllers.yaml'
    return ConfigPaths(
        gazebo_xacro=desc_share / 'urdf' / 'ur5e.urdf.xacro',
        ...
        pick_targets=bringup_share / 'config' / 'pick_targets.yaml',
        moveit_package_xml=mv_share / 'package.xml',
    )
...
def ros2_control_mimic_multiplier(root: ET.Element, joint_name: str) -> float | None:
    for block in ros2_control_blocks(root):
        for joint in block.findall('joint'):
            if joint.get('name') != joint_name:
                continue
            for param in joint.findall('param'):
                if param.get('name') == 'multiplier' and param.text:
                    return float(param.text.strip())
    return None


def urdf_joint_mimic_multiplier(root: ET.Element, joint_name: str) -> float | None:
    for joint in root.findall('joint'):
        if joint.get('name') != joint_name:
            continue
        mimic = joint.find('mimic')
        if mimic is not None and mimic.get('multiplier') is not None:
            return float(mimic.get('multiplier'))
    return None


def validate_gripper_mimic(gazebo_root: ET.Element, result: ValidationResult) -> None:
    rc_mult = ros2_control_mimic_multiplier(gazebo_root, RIGHT_KNUCKLE_JOINT)
    urdf_mult = urdf_joint_mimic_multiplier(gazebo_root, RIGHT_KNUCKLE_JOINT)
    if rc_mult is None:
        result.error(
            f'Gazebo URDF ros2_control missing mimic multiplier on {RIGHT_KNUCKLE_JOINT}',
            'ros2_control',
        )
    elif rc_mult != EXPECTED_MIMIC_MULTIPLIER:
        result.error(
            f'{RIGHT_KNUCKLE_JOINT} ros2_control mimic multiplier {rc_mult} '
            f'!= expected {EXPECTED_MIMIC_MULTIPLIER}',
            'ros2_control',
        )
    if urdf_mult is not None and urdf_mult != EXPECTED_MIMIC_MULTIPLIER:
        result.warn(
            f'{RIGHT_KNUCKLE_JOINT} URDF joint mimic multiplier {urdf_mult} '
            f'!= expected {EXPECTED_MIMIC_MULTIPLIER}',
            'urdf',
        )
    if rc_mult is not None and urdf_mult is not None and rc_mult != urdf_mult:
        result.error(
            f'{RIGHT_KNUCKLE_JOINT} ros2_control multiplier {rc_mult} '
            f'does not match URDF mimic multiplier {urdf_mult}',
            'mimic',
        )
...
def validate_joint_consistency(
    ...
    sim_arm, sim_gripper = demo_arm, demo_gripper
    ...
    if set(demo_arm) != ARM_JOINTS:
        result.error(
            f'ros2_controllers arm joints {demo_arm} != expected {sorted(ARM_JOINTS)}',
            'ros2_controllers.yaml',
        )
    if demo_gripper != [GRIPPER_CONTROLLER_JOINT]:
        result.error(
            f'ros2_controllers gripper joints {demo_gripper} != [{GRIPPER_CONTROLLER_JOINT}]',
            'ros2_controllers.yaml',
        )
    if demo_arm != sim_arm or demo_gripper != sim_gripper:
        result.error(
            'ros2_controllers.yaml paths resolve to different content for sim vs demo',
            'controllers',
        )
...
def validate_antipatterns(
    ...
    if paths.moveit_package_xml.is_file():
        ...
        for dep in ('ur_description', 'robotiq_description', 'ur5e_description'):
...
def validate_all(paths: ConfigPaths | None = None) -> ValidationResult:
    ...
    try:
        gazebo_xml, gazebo_stderr = expand_xacro(
            paths.gazebo_xacro, ['use_mock_hardware:=false']
        )
    ...
    if result.ok or gazebo_root is not None:
        ...
        validate_gripper_mimic(gazebo_root, result)
